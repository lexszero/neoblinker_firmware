...
typedef  NeoPixelBrightnessBus<NeoGrbFeature, NeoEsp8266Uart0800KbpsMethod>      NeoPBBGRBU800;
...
typedef  NeoPixelBrightnessBus<NeoRgbFeature, NeoEsp8266Uart0800KbpsMethod>      NeoPBBRGBU800;
...
typedef  NeoPixelBrightnessBus<NeoBrgFeature, NeoEsp8266Uart0800KbpsMethod>      NeoPBBBRGU800;
...
typedef  NeoPixelBrightnessBus<NeoRbgFeature, NeoEsp8266Uart0800KbpsMethod>      NeoPBBRBGU800;
...
// that use a lot of flash with mode_static (reduces flash footprint by about 3600 bytes)
  ...
  void setMode(uint8_t seg, uint8_t m) {
    RESET_RUNTIME;
  ...
  void setSpeed(uint8_t seg, uint16_t s) {
    RESET_RUNTIME;
  ...
  void setColor(uint8_t seg, uint8_t n, const RgbColor& c) {
    RESET_RUNTIME;
    _segments[seg].colors[n] = c;
  }
  ...
  void setColor(uint32_t c) {
	setColor(0, 0, (RgbColor) HtmlColor((uint32_t) c));
  }
  ...
  void setSegment(uint8_t n, uint16_t start, uint16_t stop, uint8_t mode, const RgbColor& color, uint16_t speed, bool reverse) {
  ...
  void setSegment(uint8_t n, uint16_t start, uint16_t stop, uint8_t mode, const RgbColor colors[], uint16_t speed, bool reverse) {
    if(n < (sizeof(_segments) / sizeof(_segments[0]))) {
      if(n + 1 > _num_segments) _num_segments = n + 1;
      _segments[n].start = start;
      _segments[n].stop = stop;
  ...
  void copySegmentState(uint8_t src, uint8_t dst) {
    RESET_RUNTIME;
    _segments[dst].mode = _segments[src].mode;
    _segments[dst].speed = _segments[src].speed;
    for (uint8_t i=0; i<NUM_COLORS; i++) {
      _segments[dst].colors[i] = _segments[src].colors[i];
    }
  }
